...
为 Grok Build 打造的 Windows 桌面空间，窗外是东京的雨夜。
...
- **Windows x64**。当前构建和 CI 面向 Windows，客户端暂不支持 macOS 与 Linux。
- **Node.js 22.12.0 或更高版本**及 npm。
- **Git**、已安装的 [Grok Build CLI](https://github.com/xai-org/grok-build#installing-the-released-binary)，以及能够使用该服务的账户。
...
1. 默认寻找 `%USERPROFILE%\.grok\bin\grok.exe`。如果安装在其他位置，在左下角**偏好设置**中选择它。
2. 在**账户切换**中登录或添加账户。本机账户直接沿用官方 CLI 已有的登录与配置。
3. 在**偏好设置**中选择项目目录，再开启新对话。未设置时，客户端在源码目录中创建 `Workspace` 子目录；已有会话保留各自原来的工作目录。
...
运行或分发时，请保留**整个 `App` 文件夹**。构建只纳入明确列出的运行文件和许可证，旧构建保存在 `work/package-backups`。安装依赖和首次构建需联网下载 Electron。源码仓库不附带预编译程序。
...
`data/` 保存聊天、附件、设置，以及 CLI 写入的账户凭据。它存放在本地、被 Git 忽略，**本应用不额外加密这些文件**。本机账户也可能使用 `GROK_HOME`（默认 `~/.grok`）。独立账户隔离配置和历史，不提供操作系统级沙箱。

| 仓库包含 | 不上传 Git |
| --- | --- |
| `src/`、`scripts/`、`tests/`、`docs/`、许可证与项目配置 | `data/`、`Workspace/`、`App/`、`work/`、`node_modules/`、`.env` 文件 |
...
默认测试使用隔离环境，不发送真实模型请求。Windows CI 还会检查账户、附件、语言、时钟、依赖和打包后的应用。[贡献指南](CONTRIBUTING.zh-CN.md)包含完整检查命令、贡献流程及需主动开启的真实模型测试说明。
