...
| **跟随官方引擎的控制** | 自动更新可选模型，选择 CLI 提供的推理档位，设置子代理与工作目录。 |
| **七种界面语言** | 中文、English、日本語、한국어、Español、Deutsch、Français。 |

已连接的 CLI 通知模型列表变化时，新对话的可选模型会自动更新。如果 CLI 支持会话恢复，已载入的会话也会在当前回复或设置操作结束后刷新各自的可选模型。刷新保留未发送的草稿，无需重启或发送消息；每个会话仍以 CLI 确认的可选模型和当前选择为准。
