...
When the connected CLI announces a changed model list, new-conversation choices update automatically. If the CLI supports session restoration, loaded conversations refresh their own available models after any current reply or settings operation finishes. Each conversation retains the choices and current selections confirmed for it by the CLI. The refresh preserves unsent drafts and requires neither restarting the engine nor sending a message. Model availability still depends on your CLI and account.
