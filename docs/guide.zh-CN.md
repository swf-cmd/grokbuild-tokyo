...
附件会复制到客户端管理的账户目录，原文件移动后已发送的副本仍可用。此前核查的 CLI 1.0.13 没有 ACP 原生图片输入，客户端发送本地资源引用并让官方 `read_file` 工具读取图片与文件；该方式此前已在 Windows 上通过真实 PNG 与 TXT 读取测试，这不代表已在 Mac 上完成真实模型验证。当 CLI 宣告支持图片输入时，客户端使用 ACP 内嵌图片。其他格式的解析能力取决于 CLI 可用工具与权限。
...
如果推理下拉框显示“请更新 Grok CLI”，说明引擎声明支持推理强度，但未提供可选档位。请在终端运行 `grok update`（有多个安装时，使用偏好设置中选定的可执行文件），然后点击连接状态按钮重新连接。实测 CLI 0.2.11 不返回档位列表，切换时虽然确认成功，重新载入会话却不返回所选值；Mac 更新至 1.0.34 后恢复可选档位。客户端不会编造档位，也不会仅凭成功应答就认定切换已生效。
