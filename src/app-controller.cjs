...
class AppController extends EventEmitter {
  constructor({ root, home, Adapter = GrokAdapter, Accounts = AccountManager }) {
    ...
    this.settings = { executable: path.join(home, '.grok', 'bin', 'grok.exe'), workspace: path.join(root, 'Workspace'), rainEnabled: true, musicEnabled: true, musicVolume: 90, subagentsEnabled: true };
    this.sessions = [];
    this.accounts = [{ id: 'local', name: '本机 Grok 账户' }];
    this.activeAccountId = 'local';
    this.accountManager = new Accounts({ dir: this.dir, home });
    ...
    if (fs.existsSync(this.file)) {
      try {
        ...
        if (data.accounts !== undefined) {
          ...
          this.accounts = data.accounts.map(({ id, name }) => ({ id, name }));
        ...
        this.loadError = `历史记录文件无法读取，已保留备份：${backup}`;
      }
    }
    ...
    if (!this.loadError) this.loadError = this.accountManager.recoverDeletes?.(this.accounts.map(a => a.id)).join('\n') || null;
      ...
      try {
        if (this.accountManager.preserveUnverifiedDeletes?.()) this.loadError += '\n未确认删除的账户数据已隔离至 data/accounts/.recovery-*，不会自动删除，请结合历史备份恢复。';
      } catch {
        this.accountRecoveryError = '未能隔离未确认删除的账户数据。为保留恢复依据，暂未覆盖历史文件；请检查 data/accounts 文件夹权限后重启。';
  ...
  scheduleSave() { clearTimeout(this.saveTimer); this.saveTimer = setTimeout(() => { try { this.save(); } catch (e) { this.emitEvent({ type: 'error', message: `保存失败：${e.message}` }); } }, 350); }
  visibleSessions() { return this.sessions.filter(s => (s.accountId || 'local') === this.activeAccountId); }
  getSession(id) { const s = this.visibleSessions().find(s => s.id === id); if (!s) throw new Error('会话不存在或属于其他账户'); return s; }
  ...
  accountName(name, exceptId) {
    if (typeof name !== 'string' || !name.trim() || Array.from(name.trim()).length > 60) throw new Error('请输入 1 到 60 字的账户名称');
    const trimmed = name.trim();
    const comparable = value => value.normalize('NFKC').toLowerCase();
    if (this.accounts.some(account => account.id !== exceptId && comparable(account.name) === comparable(trimmed))) throw new Error('已存在同名账户，请使用其他名称');
    return trimmed;
  }
  async accountOperation(name, action) {
    return this.idleOperation(name, () => {
      if (this.accountManager.pending) throw new Error('请先完成或取消账户登录');
      return action();
    });
  }
  async addAccount({ name } = {}) {
    return this.accountOperation('添加账户', async () => {
  ...
  async deleteAccount(id) {
    return this.accountOperation('删除账户', async () => {
      if (!this.accounts.some(a => a.id === id)) throw new Error('账户不存在');
      if (id === 'local') throw new Error('本机 Grok 账户不能删除，新增账户可以移除');
      ...
      try { this.save(); }
        ...
        if (staged) {
          ...
          catch { throw new Error(`账户删除未保存，本地数据已保留，重启应用后会尝试恢复。保存错误：${error.message}`); }
        }
        throw error;
      }
      const errors = [];
      if (staged) {
        ...
        catch { errors.push('账户已移除，但登录凭据或缓存尚未完全清理；请检查 data/accounts 文件夹权限并重启应用以重试清理。'); }
      }
      ...
      if (wasActive) {
        ...
        } catch (error) { errors.push(`账户已移除，已切回本机账户。${error.message}`); }
      }
      return this.snapshot(errors.join('\n') || null);
    });
  }
  async switchAccount(id) {
    return this.idleOperation('切换账户', async () => {
      if (!this.accounts.some(a => a.id === id)) throw new Error('账户不存在');
      if (this.accountManager.pending) throw new Error('请先完成或取消账户登录');
      this.invalidateConnection();
      const adapter = this.adapter;
  ...
  async loginAccount(id) {
    return this.idleOperation('启动账户登录', async () => {
      const account = this.accounts.find(a => a.id === id);
      if (!account) throw new Error('账户不存在');
      if (this.accountManager.pending) throw new Error('已有账户正在登录');
      if (!isPathType(this.settings.executable, 'isFile')) throw new Error('请在设置中选择有效的 grok.exe');
  ...
  async readImage({ sessionId, src } = {}) {
    ...
    try { return await resolveImage(src, session.cwd, cache); }
    catch (error) { throw new Error(error.code === 'ENOENT' ? '找不到图片文件，文件可能已移动或删除' : error.message); }
  }
  async idleOperation(name, action) {
    if (this.closing) throw new Error('应用正在关闭');
    if (this.active) throw new Error('请等待当前回复完成，或先停止生成。');
    if (this.operation) throw new Error(`正在${this.operation.name}，请稍后再试。`);
  ...
  async connect() {
    return this.idleOperation('连接 Grok', () => this._connect());
  }
  async _connect() {
    if (this.accountManager.pending?.accountId === this.activeAccountId) throw new Error('请先完成账户登录');
    if (this.activeAccountId !== 'local' && !this.accountManager.summary(this.accounts.find(a => a.id === this.activeAccountId)).signedIn) throw new Error('请打开左侧“账户切换”，登录当前账户。');
    if (this.connected) return this.normalizeInfo();
    if (this.connecting) return this.connecting;
    this.connecting = (async () => {
      if (!isPathType(this.settings.executable, 'isFile')) throw new Error('没有找到 Grok。请在设置中选择 grok.exe。');
      if (!isPathType(this.settings.workspace, 'isDirectory')) throw new Error('工作目录不存在，请在设置中重新选择。');
      if (this.adapter) await this.adapter.close();
      this.loaded.clear();
      this.adapter = new this.Adapter({ executable: this.settings.executable, cwd: this.settings.workspace, subagentsEnabled: this.settings.subagentsEnabled, env: this.accountManager.environment(this.activeAccountId) });
      const adapter = this.adapter;
  ...
  async bootstrap() {
    return this.idleOperation('初始化 Grok', async () => {
  ...
  async createSession(options = {}) {
    return this.idleOperation('创建会话', () => this._createSession(options));
  }
  async _createSession({ cwd, model, mode } = {}) {
    await this._connect();
    cwd = cwd || this.settings.workspace;
    if (!isPathType(cwd, 'isDirectory')) throw new Error('请选择有效的工作目录');
    const created = await this.adapter.newSession({ cwd, model, mode });
    const session = { id: created.sessionId, accountId: this.activeAccountId, title: '新会话', cwd, createdAt: Date.now(), updatedAt: Date.now(), messages: [] };
  ...
  async ensureLoaded(session) {
    return this.idleOperation('载入会话', () => this._ensureLoaded(session));
  }
  async _ensureLoaded(session) {
    if ((session.accountId || 'local') !== this.activeAccountId) throw new Error('请先切换到这段对话所属的账户');
    await this._connect();
    try {
  ...
  async configureSession({ sessionId, model, mode } = {}) {
    return this.idleOperation('更新会话配置', async () => {
      const session = this.getSession(sessionId);
      if (model !== undefined && (typeof model !== 'string' || !model.trim())) throw new Error('请选择有效的模型');
      if (mode !== undefined && (typeof mode !== 'string' || !mode.trim())) throw new Error('请选择有效的推理档位');
  ...
  async send({ sessionId, text }) {
    if (this.active) throw new Error('已有回复正在生成');
    if (this.closing) throw new Error('应用正在关闭');
    if (this.operation) throw new Error(`正在${this.operation.name}，请稍后再发送。`);
    if (typeof text !== 'string' || !text.trim()) throw new Error('请输入内容');
    if (text.length > 200000) throw new Error('消息过长，请缩短到 200,000 字以内');
    const session = this.getSession(sessionId);
    ...
    const now = Date.now();
    const previous = { messages: session.messages.length, title: session.title, updatedAt: session.updatedAt };
    ...
    if (session.title === '新会话') session.title = text.trim().replace(/\s+/g, ' ').slice(0, 32);
    ...
    try { this.save(); }
      ...
      this.active = null;
      throw new Error(`保存失败，消息尚未发送：${error.message}`);
    }
    ...
    this.turnPromise = (async () => {
      try {
        ...
        session.updatedAt = Date.now();
        try { this.save(); }
        catch (error) { this.emitEvent({ type: 'error', sessionId, message: `保存失败：${error.message}` }); }
        this.emitEvent({ type: 'session-updated', sessionId, session });
  ...
  async permission({ requestId, optionId }) {
    ...
    if (!pending || pending.responding || !pending.options.some(x => x.optionId === optionId)) throw new Error('授权请求已失效');
  ...
  async saveSettings(patch) {
    ...
    if (patch.executable !== undefined) {
      if (!isPathType(patch.executable, 'isFile') || path.extname(patch.executable).toLowerCase() !== '.exe') throw new Error('请选择有效的 grok.exe');
      next.executable = patch.executable;
    }
    if (patch.workspace !== undefined) {
      if (!isPathType(patch.workspace, 'isDirectory')) throw new Error('请选择有效的工作目录');
      next.workspace = patch.workspace;
    }
    if (patch.rainEnabled !== undefined) {
      if (typeof patch.rainEnabled !== 'boolean') throw new Error('雨景开关必须为布尔值');
      next.rainEnabled = patch.rainEnabled;
    }
    if (patch.musicEnabled !== undefined) {
      if (typeof patch.musicEnabled !== 'boolean') throw new Error('背景音乐开关必须为布尔值');
      next.musicEnabled = patch.musicEnabled;
    }
    if (patch.musicVolume !== undefined) {
      if (!Number.isInteger(patch.musicVolume) || patch.musicVolume < 0 || patch.musicVolume > 100) throw new Error('背景音乐音量必须为 0 到 100 的整数');
      next.musicVolume = patch.musicVolume;
    }
    if (patch.subagentsEnabled !== undefined) {
      if (typeof patch.subagentsEnabled !== 'boolean') throw new Error('子代理开关必须为布尔值');
    ...
    // Atmosphere changes do not use or restart Grok and remain usable mid-reply.
    if (!changed) {
      if (this.operation) throw new Error(`正在${this.operation.name}，请稍后再试。`);
      return persist();
    }
    return this.idleOperation('保存设置', persist);
  }
  async reconnect() {
    return this.idleOperation('重新连接 Grok', async () => {
      this.invalidateConnection();
      await this._connect();
  ...
  renameSession({ sessionId, title }) {
    if (typeof title !== 'string' || !title.trim()) throw new Error('请输入会话名称');
    const s = this.getSession(sessionId);
    const previous = s.title;
    s.title = title.trim().slice(0, 120);
    try { this.save(); } catch (error) { s.title = previous; throw error; }
    return s;
  }
  deleteSession(id) {
    if (this.active?.sessionId === id) throw new Error('请先停止当前回复');
    if (this.operation) throw new Error(`正在${this.operation.name}，请稍后再删除会话。`);
    this.getSession(id);
  ...
  exportMarkdown(id) {
    ...
    return `# ${s.title}\n\n工作目录：${s.cwd}\n\n` + s.messages.map(m => `## ${m.role === 'user' ? '你' : 'Grok'}\n\n${m.text || ''}${(m.images || []).map(image => `\n\n![${String(image.alt || '图片').replace(/[\[\]\\]/g, '')}](<${image.src.replace(/>/g, '%3E')}>)`).join('')}${m.error ? '\n\n错误：' + m.error : ''}\n`).join('\n');
  }
  async close() {
    this.closing = true;
    let failure;
